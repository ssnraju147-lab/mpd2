...
2. Buffer the node using one LM358 channel (pins 1/2/3; often labeled channel A) as a voltage follower:
   - Pin 3 (IN+) -> `VREF_NODE`
   - Pin 2 (IN−) -> Pin 1 (feedback)
   - Pin 1 (OUT) -> `VREF`
3. Add local filtering on `VREF`:
   - `10uF` bulk capacitor (electrolytic/tantalum) to GND (`+` to VREF, `-` to GND for polarized parts)
